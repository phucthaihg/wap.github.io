...
function createProductHeader(table) {
...
function createProductRow(table, prod){
    /*
    <tr style="user-select: auto;">
        <td id="P0001-ProductName">Macbook Air M2</td>
        <td id="P0001-ProductPrice">1500</td>
...
function createCartTable(cart){
    ...
    createCartTableHeader(table);

    if(cart && cart.items && cart.items.length){
        for(let i = 0; i < cart.items.length; i++){
            createTableItem(table, cart.items[i]);
        }
    }

    createCartTableFooter(table, cart);
}

function createCartTableHeader(table) {
...
function createTableItem(table, item){
    // <tr>
...
function createCartTableFooter(table, cart){
...
function server_orderCart(param) {
    ...
    promise
        ...
        .then(result => {
            ...
            createProductTable(result.product);
            createCartTable(result.cart);
        });
}
